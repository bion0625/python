...
max_sharpe = df.loc[df['Sharpe'] == df['Sharpe'].max()] # 샤프 지수 칼럼에서 샤프 지숫값이 제일 큰 행을 max_sharpe로 정한다.
...
df.plot.scatter(x='Risk', y='Returns', c='Sharpe', cmap='viridis', edgecolor='k', figsize=(11, 7), grid=True)
plt.scatter(x=max_sharpe['Risk'], y=max_sharpe['Returns'], c='r', marker='*', s=300) # 샤프 지수가 가장 큰 포트폴리오를 300 크기의 붉은 별표(★)로 표시한다.
plt.scatter(x=min_risk['Risk'], y=min_risk['Returns'], c='r', marker='X', s=200) # 리스크가 제일 작은 포트폴리오를 200 크기의 붉은 엑스표(X)로 표시한다.
