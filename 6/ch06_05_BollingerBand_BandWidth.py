...
df['bandwidth'] = (df['upper'] - df['lower']) / df['MA20'] * 100 # (상단밴드 - 하단밴드) / 중단밴드 X 100을 구해 bandwidth(밴드폭) 칼럼을 생성한다.
...
plt.plot(df.index, df['bandwidth'], color='m', label='bandwidth') # x좌표 df.idnex에 해당하는 bandwidth값을 y좌표로 설정해 자홍색(magenta) 실선으로 표시한다.
plt.grid(True)
plt.legend(loc='best')
plt.show()