...
class DualMomentum:
    ...
    def get_rltv_momentum(self, start_date, end_date, stock_count):
        ...
        if (result[0] is None):
            ...
            return
        start_date = result[0].strftime('%Y-%m-%d') # 2. DB에서 조회된 거래일을 %Y-%m-%d 포맷 문자열로 변환해 사용자가 입력한 조회 시작 일자 변수에 반영한다.
        ...
        rows = [] # 3. rows라는 빈 리스트를 먼저 만든 후, 나중에 2차원 리스트로 처리한다.
        ...
        for _, code in enumerate(self.mk.codes):
            ...
            returns = (new_price / old_price - 1) * 100 # 6. 해당 종목의 수익률은 returns = (new_price / old_price - 1) * 100 로 구한다.
            rows.append([code, self.mk.codes[code], old_price, new_price, returns]) # 7. 종목별로 구한 종목코드, 종목명, 구 가격, 신 가격, 수익률을 rows에 2차원 리스트 형태로 추가한다.
            
        # 상대 모멘텀 데이터프레임을 생성한 후 수익률순으로 출력
        df = pd.DataFrame(rows, columns=columns) # 8. rows 리스트를 인수로 받아서 데이터프레임을 생성한 뒤, 칼럼 5개만 갖도록 구조를 수정한다.
        df = df[['code', 'company', 'old_price', 'new_price', 'returns']]
        df = df.sort_values(by='returns', ascending=False) # 9. 상대 모멘텀 데이터프레임을 수익률(returns) 칼럼을 기준으로 내림차순으로 정렬한다.
        df = df.head(stock_count)
        df.index = pd.Index(range(stock_count)) # 10. 상대 모멘텀 데이터프레임의 인덱스를 수익률 순위로 변경한다.
